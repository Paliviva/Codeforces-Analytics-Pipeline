...
**③ 和乱码死磕一整夜**
生成的 HTML 中文全是乱码。试了各种 `meta` 标签都不管用。最后发现必须在文件最开头手动写入 UTF-8 BOM（`EF BB BF`），且文件流必须用 `std::ios::binary` 模式打开，才能阻止 Windows 把换行符偷偷转换。从此我对“字符编码”有了刻骨铭心的理解。
...
## 关于作者
我叫欧阳思琦，17 岁，就读于湘潭大学计算机相关专业。热爱用代码把数据从源头搬到屏幕上的全过程，对人工智能方向充满探索欲。如果你有数据工程、后端开发或 AI 相关的实习/研究机会，欢迎联系我聊聊 —— 我能带来的，不仅是干净的代码，还有一颗在 LNK2019 面前擦干眼泪继续往前走的决心。
